...
连接稳定性的说明：应用不会反复探测设备。未连接设备时，后台只做轻量的 USB 层检测（不打开 MTP 会话，Switch 上不会出现"会话打开/关闭"）；连接后应用通过常驻的 `mtp-helper serve` 进程保持**单个 MTP 会话**——DBI 的对象 ID 只在枚举出它们的会话内有效，保持同一会话才能正常浏览子目录，同时也避免了每次操作都触发 USB 复位。每次刷新只在单次会话内完成设备信息、存储区和根目录的读取，避免 DBI 响应器因频繁的会话开/关而重置。
